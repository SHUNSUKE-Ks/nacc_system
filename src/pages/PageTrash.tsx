...
const PageTrash: Component = () => {
  function restore(id: number) {
    ...
    if (!blog) return
    // Note: In production this would re-add to the blogs store.
    // For now, just remove from trash.
    setState('trashBlogs', (prev) => prev.filter((b) => b.id !== id))
  }

  function deletePermanent(id: number) {
    setState('trashBlogs', (prev) => prev.filter((b) => b.id !== id))
  }
  ...
  return (
    <div class="flex flex-col h-full overflow-hidden">
      <div class="flex items-center justify-between px-6 py-4 border-b border-[#e8e8e8] shrink-0">
        <div>
          <h1 class="text-lg font-bold text-[#37352f]">ごみ箱</h1>
        ...
        >
          <div class="max-w-2xl mx-auto flex flex-col gap-3">
            <For each={state.trashBlogs}>
              {(blog) => (
                <div class="bg-white border border-[#e8e8e8] rounded-xl p-4 flex items-start justify-between gap-4">
                  <div class="min-w-0">
                  ...
                  <div class="flex flex-col gap-1.5 shrink-0">
                    <button
                      class="text-xs px-3 py-1.5 rounded-lg border border-[#e8e8e8] text-[#37352f] hover:bg-[#f9f8f6] transition-colors"
                      onClick={() => restore(blog.id!)}
                    >
                      復元
                    </button>
                    <button
                      class="text-xs px-3 py-1.5 rounded-lg border border-red-200 text-red-400 hover:bg-red-50 transition-colors"
                      onClick={() => deletePermanent(blog.id!)}
                    >
                      完全削除
                    </button>
                  </div>
