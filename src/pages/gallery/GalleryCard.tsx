...
import { toggleGalleryFavorite, selectGalleryItem, galleryState } from './store'

const GalleryCard: Component<{ item: GalleryItem }> = (props) => {
  const isSelected = () => galleryState.selectedId === props.item.id
  const imgSrc = () => props.item.dataUrl ?? props.item.url ?? ''

  return (
    <div
      class="gallery-card relative rounded-2xl overflow-hidden bg-white cursor-pointer group shadow-sm"
      classList={{ 'ring-2 ring-violet-500 ring-offset-2': isSelected() }}
      onClick={() => selectGalleryItem(props.item.id === galleryState.selectedId ? null : props.item.id)}
    >
      {/* Square image */}
      ...
      >
        <span
          ...
          classList={{
            'text-red-400': props.item.isFavorite,
            'text-white/60': !props.item.isFavorite,
          }}
        >
      ...
      <div class="absolute bottom-0 left-0 right-0 p-2.5 pointer-events-none">
        ...
        <p class="text-white text-xs font-semibold mt-1 truncate leading-tight">
          {props.item.label}
        </p>
